...
def cancel_mirror(update, context):
    ...
    if len(args) > 1:
        ...
        if not dl:
            sendMessage(f"Batal: <code>{gid}</code> Tidak ditemukan.", context.bot, update)
            return
        ...
        with download_dict_lock:
            ...
            try:
                ...
                pass
    if len(args) == 1:
        msg = f"Harap balas <code>/{BotCommands.MirrorCommand}</code> pesan yang digunakan untuk memulai unduhan atau mengirim <code>/{BotCommands.CancelMirror} GID</code> untuk membatalkannya!"
    ...
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("Arsip sedang berlangsung, Anda tidak dapat membatalkannya.", context.bot, update)
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    sendMessage(f'{count} Unduh.(s) telah dibatalkan!', context.bot, update)
